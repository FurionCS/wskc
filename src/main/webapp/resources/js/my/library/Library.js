...
$("#sole").bsSuggest({
    ...
    url: '../sole/SoleSearch?q=', /*优先从url ajax 请求 json 帮助数据，注意最后一个参数为关键字请求参数*/
//     jsonp: 'callback',               //如果从 url 获取数据，并且需要跨域，则该参数必须设置
    ...
    fnProcessData: function(json){
        ...
        for (index = 0; index < json.obj.length; index++) {
            data.value.push({
                "Id": json.obj[index].soleNo+"  "+json.obj[index].productName,
...
$("#allocation").bsSuggest({
    ...
    effectiveFields: ["Id","brandName"],
    effectiveFieldsAlias:{Id: "编码-产品名称", brandName: "品牌"},
    url: '../allocation/AllocationSearch?type=2&q=', /*优先从url ajax 请求 json 帮助数据，注意最后一个参数为关键字请求参数*/
//     jsonp: 'callback',               //如果从 url 获取数据，并且需要跨域，则该参数必须设置
    // url 获取数据时，对数据的处理，作为 fnGetData 的回调函数
    fnProcessData: function(json){
        var index, len, data = {value: []}; 
...
function submitLibrary(){
	...
	if(brandId==""||brandId<0 || productId=="" ||productId<0){
		noty({text:"请选择对应产品,或关联销售单,调拨单",layout:'topCenter',type:"error",timeout:3000})
		return;
	}
		...
		success:function(data){
			if(data!=null){
				if(data.result==1){
					...
					setTimeout(function(){window.location.href="../library/LibraryList?menuids=5_2"},2000);
				}else{
